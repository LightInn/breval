...
describe('Blog Services (src/services/blog.services.js)', () => {
  ...
  describe('getAllBlogs', () => {
    test('should initialize Strapi and call find with correct parameters', async () => {
      ...
      expect(Strapi).toHaveBeenCalledWith({ url: 'https://breval-api.lightin.io' });
      expect(mockStrapiFind).toHaveBeenCalledTimes(1);
      expect(mockStrapiFind).toHaveBeenCalledWith('blogs', { populate: 'image' });
    });
    ...
    test('should correctly map blog data from Strapi response', async () => {
      const mockRawBlogData = [
        {
          id: 1,
          attributes: { // Strapi v4 typically wraps data in attributes
      ...
      const mockServiceInputData = mockRawBlogData.map(item => ({...item.attributes, id: item.id}));
