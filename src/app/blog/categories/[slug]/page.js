...
export async function generateMetadata({params}) {
    const category = params.slug === 'all' ? 'web development' : params.slug.replace(/-/g, ' ');
    return {
...
const CategoryPage = async ({params}) => {
    ...
    return (<article className="text-dark mt-12 flex flex-col">
        ...
        <div
            className="mt-5 grid grid-cols-1 grid-rows-2 gap-16 px-5 sm:mt-10 sm:grid-cols-2 sm:px-10 md:mt-24 md:px-24 lg:grid-cols-3 sxl:mt-32 sxl:px-32">
            {blogs.map((blog, index) => (<article className="relative col-span-1 row-span-1" key={blog.id || index}>
