...
import {
    watchViewport, unwatchViewport, getViewportState, recalibrateOrientation
} from 'tornis';
...
export default function Avatar() {
    ...
    const updateValues = ({size, scroll, mouse, position, orientation, devicePixelRatio}) => {
        if (mouse.changed) {
            ...
            // eyes
            const eyeClamp = 7;
            document.getElementById("eyeR").style.transform = `translate(${Math.min(eyeClamp, Math.max(-eyeClamp, x / changeSlowliness))}px, ${Math.min(eyeClamp, Math.max(-eyeClamp, y / changeSlowliness))}px)`;
            ...
            document.getElementById("glasses").style.transform = `translate(${Math.min(glassesClamp, Math.max(-glassesClamp, x / changeSlowliness))}px, ${Math.min(glassesClamp, Math.max(-glassesClamp, y / changeSlowliness))}px)`;

            // eyesbrows
            const eyebrowsClamp = 2;
            document.getElementById("eayleachR").style.transform = `translate(${Math.min(eyebrowsClamp, Math.max(-eyebrowsClamp, x / changeSlowliness))}px, ${Math.min(eyebrowsClamp, Math.max(-eyebrowsClamp, y / changeSlowliness))}px)`;
            document.getElementById("eayleachL").style.transform = `translate(${Math.min(eyebrowsClamp, Math.max(-eyebrowsClamp, x / changeSlowliness))}px, ${Math.min(eyebrowsClamp, Math.max(-eyebrowsClamp, y / changeSlowliness))}px)`;

            // shadow
            const shadowClamp = 2;
            document.getElementById("noseShadow").style.transform = `translate(${Math.min(shadowClamp, Math.max(-shadowClamp, x / changeSlowliness))}px, ${Math.min(shadowClamp, Math.max(-shadowClamp, y / changeSlowliness))}px)`;
            document.getElementById("eyeShadow").style.transform = `translate(${Math.min(shadowClamp, Math.max(-shadowClamp, x / changeSlowliness))}px, ${Math.min(shadowClamp, Math.max(-shadowClamp, y / changeSlowliness))}px)`;
            document.getElementById("faceShadow").style.transform = `translate(${Math.min(shadowClamp, Math.max(-shadowClamp, x / changeSlowliness))}px, ${Math.min(shadowClamp, Math.max(-shadowClamp, y / changeSlowliness))}px)`;

            // head
            const headClamp = 1;
            document.getElementById("head").style.transform = `translate(${Math.min(headClamp, Math.max(-headClamp, x / changeSlowliness))}px, ${Math.min(headClamp, Math.max(-headClamp, y / changeSlowliness))}px)`;
            document.getElementById("head").style.transform = `rotate(${Math.min(headClamp, Math.max(-headClamp, y / changeSlowliness))}deg)`;
            document.getElementById("neckShadow").style.transform = `rotate(${Math.min(headClamp, Math.max(-headClamp, y / changeSlowliness))}deg)`;


            // other
            const otherClamp = 1;
            document.getElementById("mouth2").style.transform = `translate(${Math.min(otherClamp, Math.max(-otherClamp, x / changeSlowliness))}px, ${Math.min(otherClamp, Math.max(-otherClamp, y / changeSlowliness))}px)`;
            document.getElementById("mouth2Shadow").style.transform = `translate(${Math.min(otherClamp, Math.max(-otherClamp, x / changeSlowliness))}px, ${Math.min(otherClamp, Math.max(-otherClamp, y / changeSlowliness))}px)`;
            document.getElementById("bear").style.transform = `translate(0px, ${Math.min(otherClamp, Math.max(-otherClamp, y / changeSlowliness))}px)`;
            document.getElementById("mustache").style.transform = `translate(${Math.min(otherClamp, Math.max(-otherClamp, x / changeSlowliness))}px, ${Math.min(otherClamp, Math.max(-otherClamp, y / changeSlowliness))}px)`;

            document.getElementById("particles").style.transform = `translate(${Math.min(eyeClamp, Math.max(-eyeClamp, x / changeSlowliness))}px, ${Math.min(eyeClamp, Math.max(-eyeClamp, y / changeSlowliness))}px)`;
    ...
    return (<>
        ...
        <svg className="max-w-[50vw] max-h-[80vh] overflow-visible motion-reduce:hidden hidden md:block" viewBox="0 0 687 834"
             fill="none"
            ...
            <g id="avatar" clipPath="url(#clip0_7_105)">
                ...
                <g id="particles">
                    ...
                    <circle id="p23" cx="220.676" cy="656.676" r="3.82329"
                            transform="rotate(5.57136 220.676 656.676)"
                            fill="white"/>
                    <circle id="p22" cx="99.1765" cy="729.176" r="3.82329"
                            transform="rotate(5.57136 99.1765 729.176)"
                            fill="white"/>
                    <circle id="p21" cx="268.677" cy="760.676" r="1.70076"
                            transform="rotate(5.57136 268.677 760.676)"
                            fill="white"/>
                    <circle id="p20" cx="198.358" cy="740.358" r="1.70076"
                            transform="rotate(5.57136 198.358 740.358)"
                            fill="white"/>
                    <circle id="p19" cx="333.214" cy="697.214" r="2.97906"
                            transform="rotate(4.70709 333.214 697.214)"
                            fill="white"/>
                    <circle id="p18" cx="435.813" cy="776.813" r="1.68083"
                            transform="rotate(4.70709 435.813 776.813)"
                            fill="white"/>
                    <circle id="p17" cx="571.313" cy="690.313" r="1.68083"
                            transform="rotate(4.70709 571.313 690.313)"
                            fill="white"/>
                    <circle id="p16" cx="621.313" cy="747.313" r="1.68083"
                            transform="rotate(4.70709 621.313 747.313)"
                            fill="white"/>
                    <circle id="p15" cx="89.6765" cy="645.676" r="0.794449"
                            transform="rotate(7.30395 89.6765 645.676)"
                            fill="white"/>
                    <circle id="p14" cx="249.389" cy="694.389" r="0.794449"
                            transform="rotate(7.30395 249.389 694.389)"
                            fill="white"/>
                    <circle id="p13" cx="63.3891" cy="688.389" r="2.84557"
                            transform="rotate(7.30395 63.3891 688.389)"
                            fill="white"/>
                    <circle id="p12" cx="100.694" cy="628.694" r="4.5" transform="rotate(99.706 100.694 628.694)"
                            fill="white"/>
                    <circle id="p11" cx="225.23" cy="684.494" r="7.5" transform="rotate(99.706 225.23 684.494)"
                            fill="white"/>
                    <circle id="p10" cx="473.445" cy="592.644" r="8.75202"
                            transform="rotate(100.038 473.445 592.644)"
                            fill="white"/>
                    <circle id="p9" cx="577.412" cy="715.412" r="5.10098"
                            transform="rotate(100.038 577.412 715.412)"
                            fill="white"/>
                    <circle id="p8" cx="581.412" cy="699.412" r="2.7075" transform="rotate(100.038 581.412 699.412)"
                            fill="white"/>
                    <circle id="p7" cx="620.638" cy="687.638" r="1.63513"
                            transform="rotate(100.038 620.638 687.638)"
                            fill="white"/>
                    <circle id="p6" cx="627.667" cy="652.266" r="0.895227"
                            transform="rotate(100.038 627.667 652.266)"
                            fill="white"/>
                    <circle id="p5" cx="523.469" cy="636.416" r="14.3783"
                            transform="rotate(100.038 523.469 636.416)"
                            fill="white"/>
                    <circle id="p4" cx="455.696" cy="669.696" r="5.6263"
                            transform="rotate(-160.256 455.696 669.696)"
                            fill="white"/>
                    <circle id="p3" cx="375.696" cy="546.696" r="3.01429"
                            transform="rotate(-160.256 375.696 546.696)"
                            fill="white"/>
                    <circle id="p2" cx="214.355" cy="544.355" r="2.06736"
                            transform="rotate(-160.256 214.355 544.355)"
                            fill="white"/>
                    <circle id="p1" cx="418.494" cy="694.494" r="9.37716"
                ...
                <g id="head">
                    ...
                    <path id="eyR"
                          ...
                          fill="#74341C"/>
                    <g id="glasses">
                        <path id="glassEye" fillRule="evenodd" clipRule="evenodd"
                              ...
                              fill="#DC9442"/>
                        <path id="glassEye_2" fillRule="evenodd" clipRule="evenodd"
                    ...
                    <g id="Mia">
                        <g id="miaEar">
                            <path id="Vector 25" d="M414.5 60.5L474.5 2L456.5 106" stroke="black" strokeWidth="5"/>
                            <path id="Vector 26" d="M279.5 61L205 2L222 101.5" stroke="black" strokeWidth="5"/>
                        </g>
                        <g id="miaMustache">
                            <path id="Vector 19" d="M406 267L571 201" stroke="black" strokeWidth="5"/>
                            <path id="Vector 20" d="M411 281.074L588.704 282.547" stroke="black" strokeWidth="5"/>
                            <path id="Vector 21" d="M411 296.912L580.28 350.998" stroke="black" strokeWidth="5"/>
                            <path id="Vector 22" d="M290.296 265L124.296 199" stroke="black" strokeWidth="5"/>
                            <path id="Vector 23" d="M285.252 279.074L106 280.547" stroke="black" strokeWidth="5"/>
                            <path id="Vector 24" d="M285.252 294.912L114.498 348.998" stroke="black"
                                  strokeWidth="5"/>
                        </g>
                    </g>
                </g>
